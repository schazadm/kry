...
public class BigInteger
    ...
    /**
     * <h2>serie2 a3</h2>
     * <p>Bei 16 Bit zufälligen Zahlen braucht die naive Version schon deutlich länger.</p>
     *
     * <p>
    ...
    public BigInteger myModPow(BigInteger exponent, BigInteger m) {
        ...
        BigInteger currentNumber = this;
        int exponentBits = exponent.bitLength();
        int i = 0;
        while (i < exponentBits) {
            if (exponent.testBit(i)) {
                result = result.multiply(currentNumber).mod(m);
            }
            currentNumber = currentNumber.square().mod(m);
            i++;
        }
        return result.mod(m);
    }
    ...
    public static BigInteger myProbableSafePrime(int bitLength, int certainty, Random rnd) {
        if (bitLength < 2) {
            throw new ArithmeticException("Bit length must be at least 2.");
        }

        BigInteger primeCandidate;
        do {
            primeCandidate = new BigInteger(bitLength, certainty, rnd);
        } while (!primeCandidate.subtract(BigInteger.ONE).shiftRight(1).isProbablePrime(certainty));
