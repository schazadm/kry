...
// ContentPane
    ...
    private void doDatenbankVerschl() {
        ...
        for (int i = 0; i < datenSatzUnverschl.length; i++) {
            BigInteger f = datenSatzUnverschl[i];
            BigInteger p = BigInteger.probablePrime(f.bitLength() + 1, new Random());
            schluessel[i] = p;
        }
        ...
        for (BigInteger bigInteger : schluessel) {
            m = m.multiply(bigInteger);
        }
